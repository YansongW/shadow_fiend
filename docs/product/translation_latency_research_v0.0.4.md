...
## 6. ja→zh / ko→zh 直接模型调研

在 Hugging Face 上搜索发现：

- **Helsinki-NLP 官方没有 opus-mt-ja-zh 或 opus-mt-ko-zh 直接模型**
- 存在社区模型：
  - `shun89/opus-mt-ja-zh`（下载量 1,293，Apache-2.0）
  - `shun89/opus-mt-ko-zh`（下载量 107）
  - `Helsinki-NLP/opus-mt-tc-big-zh-ja`（官方中→日，不是日→中）

### 6.1 shun89 直接模型实测

测试环境与本节相同（Apple Silicon CPU）。

| 输入 | shun89 ja→zh 直接 | pivot ja→en→zh |
|---|---|---|
| こんにちは | 你好, | 你好,我叫艾莉森 |
| ありがとう | 谢谢你们。 | 谢谢 |
| すみません | 对不起 | 对不起 |
| さようなら | 再见, | 再见 |
| これは本当に美味しいですね | 这真的是很好吃啊 | 这真的很好吃 |

| 输入 | shun89 ko→zh 直接 | pivot ko→en→zh |
|---|---|---|
| 안녕하세요 | 你好, | 你好,我叫艾莉森 |
| 감사합니다 | 多谢 | 谢谢 |
| 죄송합니다 | 对不起, | 对不起 |
| 안녕히 가세요 | 再见,去吧。 | 再见 |
| 이 영화 정말 재미있어요 | 这部电影真有趣。 | 这部电影真的很有趣 |

### 6.2 直接模型的问题

1. **首次推理异常慢**：shun89/opus-mt-ja-zh 首次翻译耗时约 **125 秒**，ko→zh 也类似
   - 模型大小约 600MB，与官方模型相当
   - 后续推理正常（60–170 ms）
   - 原因待查，可能是 tokenizer 配置或模型结构与标准 MarianMT 有差异
2. **翻译质量不稳定**：
   - "ありがとう" → "谢谢你们。"（单数语境下不自然）
   - "안녕히 가세요" → "再见,去吧。"（不自然）
3. **社区模型可信度低**：下载量小，未经验证

### 6.3 结论

- 当前没有官方直接 ja→zh / ko→zh 模型
- 社区直接模型可消除一次模型调用，但存在首次推理异常和质量不稳定问题
- **v0.0.4 不建议默认切换**，可作为实验性选项或继续观察
...
1. **动态 max_length**：根据输入 token 长度设置合理的 max_length，避免短句过度解码
2. **无标点短句保护**：检测重复 token 并截断
...
6. **ja→zh / ko→zh 直接模型**：当前仅有低下载量社区模型，且首次推理异常慢，暂不建议默认启用
...
| 步骤 | 任务 | 状态 | 备注 |
|---|---|---|---|
| 1 | 动态 max_length | ✅ 已实现 | `src/translation/opus_engine.py` |
| 2 | 重复 token 检测与截断 | ✅ 已实现 | 同上 |
| 3 | 翻译 warmup | ✅ 已实现 | 同上 |
| 4 | 高频短句缓存 | ✅ 已实现 | 同上 |
| 5 | ja→zh / ko→zh 直接模型调研 | ✅ 已完成 | 仅发现社区模型，不建议默认启用 |
| 6 | CTranslate2 实测 | ✅ 已完成 | 收益 15–38%，INT8 质量不可接受 |
| 7 | 端到端延迟基准 | ⏳ 待做 | 需在 test 分支合成集上验证 |
| 8 | 文档更新 | ⏳ 待做 | README / CHANGELOG / ROADMAP |

---

## 9. 待 PM 确认事项

1. **是否确认当前已实现的四项优化（动态 max_length、重复截断、warmup、缓存）进入 v0.0.4？**
2. **是否同意 v0.0.4 不引入 CTranslate2 和社区直接模型，作为第二阶段再评估？**
3. **是否继续做端到端延迟基准测试以量化本次优化效果？**
